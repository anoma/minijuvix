...
addConcreteInfo t c = \case
  ...
  Nothing -> PolyIdenInfo {
    _polyTypeArity = length t,
    _polyConcretes = HashMap.singleton t c
   }

buildConcreteTable :: forall r. Members '[NameIdGen] r =>
  Micro.InfoTable -> Micro.TypeCalls -> Sem r ConcreteTable
buildConcreteTable info = execState (ConcreteTable mempty)
  . mapM_ (uncurry go) . HashMap.toList . (^. Micro.typeCallSet)
  where
  go :: Members '[State ConcreteTable, NameIdGen] k =>
    Micro.TypeCallIden -> HashMap Micro.ConcreteTypeCall Micro.ConcreteSubs -> Sem k ()
  go i m = case i of
  ...
  goCFunction :: Members '[State ConcreteTable, NameIdGen] k =>
    Micro.FunctionName -> (Micro.ConcreteTypeCall, Micro.ConcreteSubs) -> Sem k ()
  goCFunction f (tc, s) = do
    f' <- cloneName f
    let fc = ConcreteIdenInfo {
          _concreteName = f',
          _concreteTypeSubs = s
        }
  ...
  goCInductive :: Members '[State ConcreteTable, NameIdGen] k =>
    Micro.InductiveName -> (Micro.ConcreteTypeCall, Micro.ConcreteSubs) -> Sem k ()
  goCInductive ind (tc, s) = do
    let
      def :: Micro.InductiveDef
    ...
    let ic = ConcreteIdenInfo {
          _concreteName = ind',
          _concreteTypeSubs = s
         }
    modify (over concreteTable (over (at iden) (Just . addConcreteInfo k ic)))
    where
    goCConstructor :: Members '[State ConcreteTable, NameIdGen] k =>
      Micro.ConstructorName -> Sem k ()
    goCConstructor c = do
      c' <- cloneName c
      let cc = ConcreteIdenInfo {
            _concreteName = c',
            _concreteTypeSubs = s
        }
          k :: NonEmpty Micro.ConcreteType
...
goModule :: Members '[Error Err, Reader ConcreteTable, NameIdGen] r =>
  Micro.Module -> Sem r Module
...
goStatement :: Members '[Error Err, Reader ConcreteTable, NameIdGen] r =>
...
goFunctionDefPoly def poly
  ...
  go i = do
    ...
    goClause funName c = do
      ...
      _clauseBody <- goExpression (Micro.substitutionE
           (concreteSubsE <> Micro.renameToSubsE localVarsRename) (c ^. Micro.clauseBody))
