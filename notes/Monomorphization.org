...
- If =n > 1=. For each =âᵢ= we proceed as follows in the next sections. Assume
  =âᵢ= is of length =m ≥ 0=.

*** Function name
 The name of the monomorphized function is =id(f âᵢ)=.
...
*** Type signature
 ...
 Then proceed as described in [[Expressions and types]].
...
*** Expressions and types
 ...
 Then if =f a₁ … aₖ ∉ ℒ= then recurse normally, otherwise, wlog assume =f= is a
 function. Replace the original expression =f a₁ … aₘ=, by =id(f â) aₖ₊₁' … aₘ'=,
 where =aₖ₊₁' … aₘ'= are the monomorphization of =aₖ₊₁ … aₘ= respectively.
